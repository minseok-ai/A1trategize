...
### PPTX 산출 로직 고도화 및 테이블 네이티브 지원
*최신 코드 변경(diff)을 통해 PPTX 문서 생성 시각화 및 안정성이 대폭 개선되었습니다.*

- **마크다운 표(Table) PPTX 네이티브 변환**: 보고서에 포함된 마크다운 표 형태(`|...|`)를 파싱하여, PPTX 슬라이드 내 실제 표 객체(Table Shape)로 변환해 주는 `_add_table_slide_optimized` 기능이 구현되었습니다.
- **테이블-텍스트 동적 페이지네이션 분리**: 텍스트와 표가 섞여 있을 때, 표 전후로 슬라이드를 분리하여 가독성을 높이고 내용이 슬라이드를 벗어나는 문제를 방지합니다.
- **한글 파일명 안전성 확보**: PPTX 파일 다운로드 시 한글 주제어가 파일명에서 누락되거나 오류를 일으키지 않도록, 정규식(`\uAC00-\uD7A3`)을 개선하여 한글명을 안전하게 보존합니다.
