...
### 전면적인 코드 문서화 및 최신 모델 업그레이드 (10개 파일 리팩토링)
*최신 코드 변경(diff)을 통해 프로젝트 전체의 유지보수성과 생성 품질이 한 단계 도약했습니다.*

- **코드 가독성 및 유지보수성 향상**: `main.py`, `utils.py`, `report_generator.py` 등 10개의 핵심 모듈 전체에 걸쳐 상세한 Docstring(함수/클래스 설명)을 추가하여, 각 파이프라인 단계와 유틸리티의 역할을 명확히 문서화했습니다.
- **최신 AI 모델 도입 (`config.py`)**: 초안 작성 및 수정 모델을 최신 `gemini-3-pro-preview`로, 리뷰 모델을 `solar-pro3`로 업그레이드하여 보고서의 추론 능력과 문장 완성도를 극대화했습니다.
- **핵심 버그 수정 및 안정화**: Perplexity API의 `citations`(인용구) 처리 로직 오류를 수정하고, 비평가(Critic) 모드에서 XML 태그만 확실하게 추출하도록 파싱 로직을 개선했습니다. 또한 데이터 충분성 평가(v3) 시 LLM 품질 점수가 낮을 경우 확실하게 재조사를 수행하도록(Hard Fail) 기준을 강화했습니다.
