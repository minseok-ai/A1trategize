...
### Link Vault & NNFC Reference Canonicalization System (v0.8)
*Introduced a Link Vault and Canonicalization engine to completely eradicate fake URLs and non-existent equipment references caused by LLM hallucinations.*

- **Link Vault System Injection**: Prohibited LLMs from directly generating URLs within the report body across Business, Career, and Patent domains. Instead, the prompt engine has been overhauled so that the backend's Link Vault system automatically injects only verified URLs post-generation, fundamentally solving the "dead link hallucination" problem.
- **NNFC Equipment Reference Canonicalization**: Upgraded the `NNFCEquipmentEngine` to not just validate equipment IDs, but automatically canonicalize mentions in the text into a standard format: `Equipment Name (ID: 123) [NNFC DB: 123]`. If fabricated IDs are detected, rather than blindly correcting them, it softens them into `[NNFC equipment reference requires engineer confirmation]` to gracefully prompt engineer review.
- **Alibaba LLM Integration & Reasoning Block Sanitization**: Officially added support for Alibaba models in the frontend router. Furthermore, it now automatically strips internal reasoning blocks (e.g., `<think>`, `<details>`, ````thinking````) generated by reasoning models like DeepSeek or Solar. This ensures that meta-text does not leak into the final generated report UI.
- **Timezone Library Modernization**: Completely removed the external `pytz` dependency, migrating all domain prompt timezone handling logic to Python 3.9+'s standard `zoneinfo.ZoneInfo` for improved performance and compatibility.
